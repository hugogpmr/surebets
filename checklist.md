...
| Betsson | API antifraude propia | 403 en `/api/fraud/v1/groupib/tokens/generate`, antes de poder pedir cuotas. **Reconfirmado 2026-09-24** con Playwright headless real: la página en sí ya carga completa (200, antes se pensaba bloqueada del todo), pero ese endpoint sigue dando 403 de forma consistente. |
...
| Suertia (OlyBet) | WAF (F5/Volterra) | "Access Denied"/"Request Rejected" (`server: volt-adc`). **Confirmado 2026-09-24**: el sportsbook real es `apuestas.olybet.es` (plataforma GiG Sport/Sportnco, cuotas server-rendered en el HTML, sin API JSON aparte); el navegador interactivo de esta sesión SÍ carga la página con cuotas reales, pero un `chromium.launch(headless=True)` real con la misma IP residencial recibe el rechazo del WAF - mismo patrón que Interwetten (el navegador interactivo no es una señal fiable de que algo no esté bloqueado). |
...
| Luckia | Cloudflare | Challenge JS "Just a moment...". **Ahora cubierta indirectamente vía CuotasAhora.com** (ver abajo). Reconfirmado 2026-09-24 con Playwright headless real, sin cambios. |
