...
| **Betway, Retabet, Paf.es, Speedybet.es, 1xBet.es** | ✅ Vía CuotasAhora.com / BetExplorer.com | No probadas directamente, cubiertas de golpe a través de los comparadores. |
...
| **Zebet** (nuevo 2026-09-24) | ✅ Funciona (`providers/zebet.py`) | Playwright headless normal, sin trucos — confirmado en vivo tras descartar que fuera un falso positivo del Browser pane (mismo patrón que Interwetten/Retabet/OlyBet). Plataforma propia del grupo Zeturf (no un B2B conocido). 1X2 sale del listado de competición con clases estables (`bet-actor1`/`bet-actorN`/`bet-actor2`, no por texto); cuotas ya en el HTML servido, no hace falta la conexión SSE que solo empuja partidos en vivo. **Doble oportunidad, Ambos marcan, Par/Impar, Más/Menos (todas las líneas, partido y por mitad) y Hándicap asiático** (añadidos el mismo día) viven en la ficha de cada partido (`data-t="..."`, identificados por su atributo o por el texto exacto de la pregunta cuando el `data-t` se reutiliza para varias variantes), lo que exige una navegación de página extra por partido — un coste real a tener en cuenta, no una API JSON barata como bet777/888sport. Esta casa también tiene un hándicap de 3 vías con notación de marcador ("Hándicap (2:0)", con empate) que se descarta a propósito: ninguna otra fuente de este repo lo emite, así que nunca podría cruzar. |
...
| Versus | ~~⚠️ Widget propio, sin explorar~~ **Resuelto 2026-09-24**: mismo framework "ta-" que Sportium/Marca Apuestas. Ver `providers/versus.py` y la fila de arriba. |
