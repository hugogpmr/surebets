...
| **Zebet** (nuevo 2026-09-24) | ✅ Funciona (`providers/zebet.py`) | Playwright headless normal, sin trucos — confirmado en vivo tras descartar que fuera un falso positivo del Browser pane (mismo patrón que Interwetten/Retabet/OlyBet). Plataforma propia del grupo Zeturf (no un B2B conocido). 1X2 sale del listado de competición con clases estables (`bet-actor1`/`bet-actorN`/`bet-actor2`, no por texto); cuotas ya en el HTML servido, no hace falta la conexión SSE que solo empuja partidos en vivo. **Doble oportunidad, Ambos marcan y Par/Impar** (añadidos el mismo día) viven en la ficha de cada partido (`data-t="..."`, identificados por etiqueta de texto), lo que exige una navegación de página extra por partido (~58 s para 20 partidos de LaLiga con los 4 mercados, frente a unos pocos segundos solo con 1X2) — un coste real a tener en cuenta, no una API JSON barata como bet777/888sport. |
